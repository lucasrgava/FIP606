...
As vezes é preciso carregar pacotes que não estão disponíveis no repóritório CRAN e utilizando a função "install.packages()" não será possível instalá-los. Para resolver isso, pode-se fazar uso de outro instalador como se segue:
...
Outra ferramente muito útil para lidar com colagem de dados diretamente no código, é instalando o pacote **datapasta**. Esse pacote oferece funcionalidades para colagem de dados de forma organizada junto ao código. Após a instalação, as funcionalidades do pacote ficam disponíves no canto superior da janela com um botão *Addins*. Clicando nele, há diversas formas de colagem de dados. Mais informações [aqui](https://cran.r-project.org/web/packages/datapasta/vignettes/how-to-datapasta.html).
...
Selecione o conjunto de dados que se deseja colar, clique na opção *Addins*, na seção DATAPASTA clique em *paste as vactor*. Altomaticamente seus dados serão colados dentro da função concatenar ("c()"). Caso queira que a colagem seja dos dados seja na vertical, há a opção *paste as vector (vertical)*.
...
É como um data frame com algumas funcionalidades a mais. Por exemplo, elas não mudam automaticamente os dados das colulas de strings para fatores. Elas podem conter culunas mais complexas como culunas de listas. Uma impressão dos dados que não sobrecarrega o console. Para colagem desse tipo de dados, semelhante ao passo anterior, procute em *Addins* a opção referente a colagem de tibble.
